...
function hypertimer(options) {
  ...
  var current = null;    // the currently handled timeout (callback is being executed)
  ...
  function _schedule() {
    ...
    if (running && next) {
      ...
      function trigger() {
        ...
        // execute a single timeout.
        function handleTimeout(timeout, callback) {
          ...
          if (current && current.type === TYPE.INTERVAL) {
            // interval is not cleared while executing the callback, reschedule it
        ...
        function recurse() {
          if (timeouts.length > 0  && ((timeouts[0].time <= time) || !isFinite(timeouts[0].time))) {
            // grab the next timeout from the queue and execute it
            handleTimeout(timeouts.shift(), recurse);
          }
          else {
