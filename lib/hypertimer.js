...
var system = {
  ...
  clearInterval: root.clearInterval
};
...
function hypertimer(options) {
    ...
    if (options) {
      ...
      if (options.global != undefined) {
        if (options.global === false) {
          // restore system date
          root.Date = system.Date;
        }

        if (options.global === true) {
          // TODO: replace Date, setTimeout, setInterval, clearTimeout, clearInterval
  ...
  /**
   * Returns the system time as Date, even when Date is replaced with the
   * hypertimers Date by configuring {global: true}.
   * @return {Date} The system time
   */
  timer.getSystemTime = function() {
