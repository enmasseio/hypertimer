...
var WebSocket = (typeof window === 'undefined' || typeof window.WebSocket === 'undefined') ? require('ws') : window.WebSocket;

var util = require('./util');
var timesync = require('timesync/dist/timesync');

// TODO: force users to load es6-shim?
...
function hypertimer(options) {
  ...
  var ws = null;                    // WebSocket, used when configured as a slave
  ...
  function _setConfig(options) {
    ...
    if ('master' in options) {
      ...
      if (typeof options.master === 'string') {
        ...
        for (var prop in options.master) {
      ...
      if (!ts) {
        ...
        timesyncOptions.server = ws;

        ts = timesync.create(timesyncOptions);
        ts.send = function (ws, data) {
        ...
        ws.onmessage = function (event) {
          //console.log('slave onmessage', event.data); // TODO: cleanup
          var msg = JSON.parse(event.data);
          // TODO: move this to a hypertimer.receive function
          if (msg.event === 'config') {
            var prev = timer.config();
            var curr = timer.config(msg.data);
            timer.emit('config', curr, prev);
          }
          else if (msg.event === 'timesync') {
            ts.receive(ws, msg.data);
          }
        };
      }
      else {
