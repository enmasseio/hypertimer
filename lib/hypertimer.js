...
function hypertimer(options) {
  ...
  function _setConfig(options) {
    ...
    if ('master' in options) {
      ...
      slave.on('change', time =>   applyConfig({time: time}));
      slave.on('config', config => applyConfig(config));
      slave.on('error',  err =>    timer.emit('error', err));
    }
    ...
    if ('port' in options) {
      ...
      master.on('error',  err => timer.emit('error', err));
    }
